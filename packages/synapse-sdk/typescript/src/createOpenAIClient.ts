...
interface OpenAIClientOptions extends Record<string, any> {
  ...
  /**
   * Whether to always use the Synapse-managed proxy for model requests
   */
  alwaysUseProxy?: boolean;

  /**
   * API key for Synapse Hub
   */
  apiKey?: string;
...
/**
 * Create and configure an OpenAI client that uses Synapse Hub for authentication
 *
    ...
    fetch: async (url, init) => {
      ...
      if (init?.method === "POST" && init?.body) {
        try {
          ...
          const synapseProperties: SynapseProperties = {
