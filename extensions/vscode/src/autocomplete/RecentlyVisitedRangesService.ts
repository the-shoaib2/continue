      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude Synapse's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in the Synapse's output
            !s.filepath.startsWith("output:extension-output-Synapse.synapse")),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
