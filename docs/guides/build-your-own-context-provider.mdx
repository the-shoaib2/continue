...
Then, create a server that responds to requests as are made from [HttpContextProvider.ts](https://github.com/continuedev/synapse/blob/main/core/context/providers/HttpContextProvider.ts). See the `hello` endpoint in [context_provider_server.py](https://github.com/continuedev/synapse/blob/main/core/context/providers/context_provider_server.py) for an example that uses FastAPI.
...
Custom context providers can be implemented in a `config.ts` placed in your Continue global directory (`~/.synapse` for MacOS, `%USERPROFILE%/.synapse` for Windows). You can implement the `CustomContextProvider` interface in your `config.ts`
...
To include outside Node modules in your config.ts, run `npm install <module_name>` from the `~/.synapse` directory, and then import them in config.ts.

Continue will use [esbuild](https://esbuild.github.io/) to bundle your `config.ts` and any dependencies into a single Javascript file. The exact configuration used can be found [here](https://github.com/continuedev/synapse/blob/5c9874400e223bbc9786a8823614a2e501fbdaf7/extensions/vscode/src/ideProtocol.ts#L45-L52).
...
```json
{ "extensionDependencies": ["synapse.synapse"] }
```
...
```json
{ "devDependencies": { "@synapse/core": "^0.0.1" } }
```
