...
@Docs uses [`docs` blocks](../../hub/blocks/block-types#docs) in Assistants from the hub. Visit the hub to [explore `docs` blocks](https://hub.synapse.dev/explore/docs) or [create your own](https://hub.synapse.dev/new?type=block&blockType=docs).
