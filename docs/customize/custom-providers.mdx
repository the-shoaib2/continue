...
You can add context providers to assistants using [`context` blocks](/hub/blocks/block-types#context). Explore available context blocks in [the hub](https://hub.synapse.dev/explore/context).
  ...
  "contextProviders": [
    {
      "name": "issue",
      "params": {
        "repos": [{ "owner": "continuedev", "repo": "synapse" }],
        "githubToken": "ghp_xxx"
      }
    }
  ]
...
Not seeing what you want? Create an issue [here](https://github.com/continuedev/synapse/issues/new?assignees=TyDunn&labels=enhancement&projects=&template=feature-request-%F0%9F%92%AA.md&title=) to request a new Context Provider.
