...
Hub Assistants can be managed in [the Hub](https://hub.synapse.dev). See [Editing an Assistant](../../hub/assistants/edit-an-assistant.md)

## YAML Config

Local user-level configuration is stored and can be edited in your home directory in `config.yaml`:

- `~/.synapse/config.yaml` (MacOS / Linux)
- `%USERPROFILE%\.synapse\config.yaml` (Windows)
...
- [`config.ts`](#configts) - Advanced configuration (probably unnecessary) - a TypeScript file in your home directory that can be used to programmatically modify (_merged_) the `config.json` schema:
  - `~/.synapse/config.ts` (MacOS / Linux)
- `%USERPROFILE%\.synapse\config.ts` (Windows)
...
`config.yaml` or `config.json` can handle the vast majority of necessary configuration, so we recommend using it whenever possible. However, if you need to programmatically extend Continue configuration, you can use a `config.ts` file, placed at `~/.synapse/config.ts` (MacOS / Linux) or `%USERPROFILE%\.synapse\config.ts` (Windows).
