...
[here](https://hub.synapse.dev/explore/mcp).
...
```yaml title=".synapse/mcpServers/playwright-mcp.yaml"
...
For complete setup instructions, troubleshooting, and usage examples, see the [Continue MCP Reference](/reference/synapse-mcp).
...
`mcpServers`
block](https://hub.synapse.dev/new?type=block&blockType=mcpServers) or add a local MCP
