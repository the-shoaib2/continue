...
keywords: [rules, .synapserules, system, prompt, message]
---
...
1. Create a folder called `.synapse/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".synapse/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.synapse/rules` based on your conversation.

</Info>

Rules can also be added to an Assistant on the Continue Hub.

Explore available rules [here](https://hub.synapse.dev), or [create your own](https://hub.synapse.dev/new?type=block&blockType=rules) in the Hub.
...
### `.synapse/rules` folder

You can create project-specific rules by adding a `.synapse/rules` folder to the root of your project and adding new rule files.
...
```md title=".synapse/rules/typescript.md"
---
...
This can be viewed in the rules section of the toolbar (see above), or in the source code [here](https://github.com/continuedev/synapse/blob/main/core/llm/constructMessages.ts#L4).
...
<Warning>
  `.synapserules` will be deprecated in a future release. Please use the
`.synapse/rules` folder instead.
</Warning>

You can create project-specific rules by adding a `.synapserules` file to the root of your project. This file is raw text and its full contents will be used as any rules.
