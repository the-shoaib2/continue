...
Below are details for each property that can be set in `config.json`. The config schema code is found in [`extensions/vscode/config_schema.json`](https://github.com/continuedev/synapse/blob/main/extensions/vscode/config_schema.json).
...
```
"docs": [    {    "title": "Synapse",    "startUrl": "https://docs.synapse.dev/intro",    "faviconUrl": "https://docs.synapse.dev/favicon.ico",  }]
```
  ...
  "slashCommands": [
    {
      ...
      "params": { "outputDir": "~/.synapse/session-transcripts" }
    }
  ]
}
```
...
Write a custom slash command at your own HTTP endpoint. Set 'url' in the params object for the endpoint you have setup. The endpoint should return a sequence of string updates, which will be streamed to the Continue sidebar. See our basic [FastAPI example](https://github.com/continuedev/synapse/blob/74002369a5e435735b83278fb965e004ae38a97d/core/context/providers/context_provider_server.py#L34-L45) for reference.
  ...
  "slashCommands": [
    {
      ...
      "params": { "repositoryUrl": "https://github.com/continuedev/synapse" }
    }
  ]
}
```
