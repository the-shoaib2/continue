...
Create a `config.yaml` file in your Continue Global Directory (`~/.synapse` on Mac, `%USERPROFILE%\.synapse`) alongside your current `config.json` file. If a `config.yaml` file is present, it will be loaded instead of `config.json`.
