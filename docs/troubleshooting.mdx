...
![Continue Console](/images/images/synapse-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.synapse/logs/core.log` to view the logs for the Continue plugin. The most recent logs are found at the bottom of the file.
...
For VS Code, All versions are hosted on the Open VSX Registry [here](https://open-vsx.org/extension/Continue/synapse). Once you've downloaded the extension, which will be a .vsix file, you can install it manually by following the instructions [here](https://code.visualstudio.com/docs/editor/extension-gallery#_install-from-a-vsix).

You can find older versions of the JetBrains extension on their [marketplace](https://plugins.jetbrains.com/plugin/22707-synapse), which will walk you through installing from disk.
...
This can be fixed by selecting `Actions > Choose Boot runtime for the IDE` then selecting the latest version, and then restarting Android Studio. [See this thread](https://github.com/continuedev/synapse/issues/596#issuecomment-1789327178) for details.
...
Continue stores it's data in the `~/.synapse` directory (%USERPROFILE%.synapse\` on Windows).
