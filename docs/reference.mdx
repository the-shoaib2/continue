...
Synapse hub assistants are defined using the `config.yaml` specification. Assistants can be loaded from [the Hub](https://hub.synapse.dev/explore/assistants) or locally

- [Synapse Hub](https://hub.synapse.dev/explore/assistants) - YAML is stored on the hub and automatically synced to the extension

- Locally
  - in your global `.synapse` folder (`~/.synapse` on Mac, `%USERPROFILE%\.synapse`) within `.synapse/assistants`. The name of the file will be used as the display name of the assistant, e.g. `My Assistant.yaml`
  - in your workspace in a `/.synapse/assistants` folder, with the same naming convention
...
Examples of blocks and assistants can be found on the [Synapse hub](https://hub.synapse.dev/explore/assistants).
...
Hub blocks and assistants are identified with a slug in the format `owner-slug/block-or-assistant-slug`, where an owner can be a user or organization (For example, if you want to use the [OpenAI 4o Model block](https://hub.synapse.dev/openai/gpt-4o), your slug would be `openai/gpt-4o`). These blocks are pulled from [https://hub.synapse.dev](https://hub.synapse.dev).
...
It is also possible to define blocks locally in a `.synapse` folder. This folder can be located at either the root of your workspace (these will automatically be applied to all assistants when you are in that workspace) or in your home directory at `~/.synapse` (these will automatically be applied globally).

Place your YAML files in the following folders:

Assistants:

- `.synapse/assistants` - for assistants

Blocks:

- `.synapse/rules` - for rules
- `.synapse/models` - for models
- `.synapse/prompts` - for prompts
- `.synapse/context` - for context providers
- `.synapse/docs` - for docs
- `.synapse/data` - for data
- `.synapse/mcpServers` - for MCP Servers

You can find many examples of each of these block types on the [Synapse Explore Page](https://hub.synapse.dev/explore/models)

<Info>
Local blocks utilizing mustache notation for secrets (`${{ secrets.SECRET_NAME }}`) can read secret values:

- globally, from a `.env` located in the global `.synapse` folder (`~/.synapse/.env`)
...
docs:
  - name: Continue
    startUrl: https://docs.synapse.dev/intro
favicon: https://docs.synapse.dev/favicon.ico
```
...
data:
  - name: Local Data Bank
    destination: file:///Users/dallin/Documents/code/continuedev/synapse-extras/external-data
    schema: 0.2.0
    level: all
