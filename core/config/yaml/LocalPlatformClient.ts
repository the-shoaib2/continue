...
export class LocalPlatformClient implements PlatformClient {
  ...
  /**
   * searches for the first valid secret file in order of ~/.synapse/.env, <workspace>/.synapse/.env, <workspace>/.env
   */
  ...
  private findSecretInLocalEnvFile(fqsn: FQSN): string | undefined {
    try {
      ...
      console.warn(
        `Error reading ~/.synapse/.env file: ${error instanceof Error ? error.message : String(error)}`,
      );
      return undefined;
    }
  }
